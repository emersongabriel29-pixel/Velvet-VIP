...
export const AdminCommandCenter: React.FC<{ onSelectVideo?: (id:string)=>void }> = ({ onSelectVideo }) => {
  ...
  const [tab,setTab] = useState<'overview'|'editor'|'plans'|'settings'|'catalog'|'moderation'|'creators'|'ads'>('overview');
  ...
  const nav=[['overview','Visão geral',<BarChart3 className="h-4 w-4"/>],['editor','Painel de edição',<Settings2 className="h-4 w-4"/>],['plans','Planos gerais',<DollarSign className="h-4 w-4"/>],['settings','Taxas e regras',<Settings className="h-4 w-4"/>],['catalog','Categorias e tags',<Tag className="h-4 w-4"/>],['moderation','Fila central',<ShieldAlert className="h-4 w-4"/>],['creators','Criadores',<Users className="h-4 w-4"/>],['ads','Campanhas',<Megaphone className="h-4 w-4"/>]] as const;
  const panel=tab==='editor'?<AdminEditor/>:tab==='overview'?<section><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[['Usuários',counts.users,Users],['Criadores',counts.creators,Users],['Vídeos',counts.videos,Eye],['Receita confirmada',`R$ ${counts.revenue.toFixed(2).replace('.',',')}`,DollarSign],['Assinaturas ativas',counts.subscribers,CheckCircle2],['Pendências de segurança',counts.pendingReports+moderation.length,ShieldAlert]].map(([label,value,Icon]:any)=><div key={label} className="rounded-3xl border border-zinc-800 bg-zinc-900/70 p-5"><Icon className="h-6 w-6 text-rose-400"/><p className="mt-4 text-xs text-zinc-500">{label}</p><p className="mt-1 text-2xl font-black">{value}</p></div>)}</div><div className="mt-5 rounded-3xl border border-zinc-800 bg-zinc-900/70 p-5"><h2 className="font-bold">Visão operacional</h2><p className="mt-2 text-sm text-zinc-400">Indicadores carregados do Supabase: usuários, criadores, vídeos, pagamentos confirmados, assinaturas, denúncias e fila de moderação.</p></div></section>
  ...
  return <div className="min-h-screen bg-[#09090b] px-4 pb-24 pt-24 text-white"><div className="mx-auto max-w-6xl"><div className="mb-5 flex items-start justify-between md:mb-8"><div><p className="text-xs font-bold uppercase tracking-[0.25em] text-rose-400">Controle operacional</p><h1 className="mt-1 text-2xl font-black sm:text-3xl">Centro do Administrador</h1><p className="mt-2 text-zinc-400">Gestão persistente de planos, taxas, catálogo, criadores, anúncios, pagamentos e segurança.</p></div><Settings className="h-8 w-8 text-zinc-600"/></div>{message&&<div role="status" className="mb-5 rounded-2xl border border-amber-500/30 bg-amber-950/20 p-4 text-sm text-amber-200">{message}</div>}<div className="mb-4 md:hidden"><label className="mb-1 block text-xs font-bold text-zinc-400">Seção administrativa</label><select value={tab} onChange={e=>setTab(e.target.value as typeof tab)} className="w-full rounded-xl border border-zinc-700 bg-zinc-900 px-3 py-3 text-sm font-bold text-white">{nav.map(([id,label])=><option key={id} value={id}>{label}</option>)}</select></div><div className="mb-6 hidden gap-2 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-2 md:grid md:grid-cols-4 lg:grid-cols-8">{nav.map(([id,label,icon])=><button key={id} onClick={()=>setTab(id)} className={`flex items-center justify-center gap-2 rounded-xl px-2 py-3 text-xs font-bold ${tab===id?'bg-rose-600 text-white':'text-zinc-400 hover:bg-zinc-800 hover:text-white'}`}>{icon}{label}</button>)}</div>{loading?<div className="flex justify-center p-12"><Loader2 className="h-6 w-6 animate-spin text-rose-400"/></div>:panel}</div></div>;
};
