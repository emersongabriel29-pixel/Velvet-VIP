...
export const AdminCommandCenter: React.FC<{ onSelectVideo?: (id:string)=>void }> = ({ onSelectVideo }) => {
  ...
  const nav=[['overview','Status operacional',<BarChart3 className="h-4 w-4"/>],['editor','Painel de edição',<Settings2 className="h-4 w-4"/>],['plans','Planos gerais',<DollarSign className="h-4 w-4"/>],['settings','Taxas e regras',<Settings className="h-4 w-4"/>],['catalog','Categorias e tags',<Tag className="h-4 w-4"/>],['moderation','Fila central',<ShieldAlert className="h-4 w-4"/>],['punishments','Punições',<ShieldAlert className="h-4 w-4"/>],['creators','Criadores',<Users className="h-4 w-4"/>],['ads','Campanhas',<Megaphone className="h-4 w-4"/>]] as const;
  ...
  <form onSubmit={saveCampaign} className="mb-5 rounded-3xl border border-zinc-800 bg-zinc-900/70 p-4 sm:p-5">
    <div className="grid gap-3 md:grid-cols-2">
      ...
      <label className="text-xs text-zinc-400">Posicionamento<select value={newCampaign.placement} onChange={e=>setNewCampaign({...newCampaign,placement:e.target.value})} className="mt-1 w-full rounded-xl border border-zinc-700 bg-zinc-950 px-3 py-2.5 text-sm"><option value="feed">Feed</option><option value="explore">Explorar</option><option value="banner">Banner</option></select></label>
