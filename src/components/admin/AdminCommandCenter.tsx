...
export const AdminCommandCenter: React.FC<{ onSelectVideo?: (id: string) => void }> = ({ onSelectVideo }) => {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const [users, creators, videos, paid, subs, reportRows, planRows] = await Promise.all([
      ...
      const error = [users, creators, videos, paid, subs, reportRows, planRows].find(result => result.error)?.error;
