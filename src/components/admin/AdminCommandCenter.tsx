...
export const AdminCommandCenter: React.FC<{ onSelectVideo?: (id:string)=>void }> = ({ onSelectVideo }) => {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const [users,creators,videos,paid,subs,reportRows,planRows,settingsRow,categoryRows,tagRows,creatorList,campaignRows,videoRows,commentRows,liveRows,health] = await Promise.all([
      ...
      const error=[users,creators,videos,paid,subs,reportRows,planRows,settingsRow,categoryRows,tagRows,creatorList,campaignRows,videoRows,commentRows,liveRows,health].find(x=>x.error)?.error;
  ...
  const panel=tab==='punishments'?<PunishmentsPanel/>:tab==='editor'?<AdminEditor/>:tab==='overview'?<section><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[['Usuários',counts.users,Users],['Criadores',counts.creators,Users],['Vídeos',counts.videos,Eye],['Receita confirmada',`R$ ${counts.revenue.toFixed(2).replace('.',',')}`,DollarSign],['Assinaturas ativas',counts.subscribers,CheckCircle2],['Pendências de segurança',counts.pendingReports+moderation.length,ShieldAlert]].map(([label,value,Icon]:any)=><div key={label} className="rounded-3xl border border-zinc-800 bg-zinc-900/70 p-5"><Icon className="h-6 w-6 text-rose-400"/><p className="mt-4 text-xs text-zinc-500">{label}</p><p className="mt-1 text-2xl font-black">{value}</p></div>)}</div><div className="mt-5 rounded-3xl border border-zinc-800 bg-zinc-900/70 p-5"><h2 className="font-bold">Visão operacional</h2><p className="mt-2 text-sm text-zinc-400">Indicadores carregados do Supabase: usuários, criadores, vídeos, pagamentos confirmados, assinaturas, denúncias e fila de moderação.</p>{ops&&<div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{[['Webhook de pagamento atrasado',ops.unprocessed_payment_events],['Mídia travada',ops.stuck_media_jobs],['Mídia com erro',ops.failed_media_jobs],['Eventos externos atrasados',ops.unprocessed_provider_events],['Alertas críticos',ops.unacknowledged_critical_alerts],['Lives sem atualização',ops.stale_live_sessions]].map(([label,value]:any)=><div key={label} className={`rounded-xl border p-3 text-xs ${Number(value)>0?'border-rose-500/30 bg-rose-950/20 text-rose-200':'border-emerald-500/20 bg-emerald-950/10 text-emerald-200'}`}><b>{value}</b> · {label}</div>)}</div>}</div></section>
