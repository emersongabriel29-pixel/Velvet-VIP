  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
    <div className="relative w-full max-w-md rounded-3xl border border-zinc-800 bg-[#121217] p-6 sm:p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
      ...
      {mode === 'login' ? <form onSubmit={submitLogin} className="space-y-4">
        ...
        <label className="block text-xs text-zinc-300">Senha<input value={password} onChange={e => setPassword(e.target.value)} type="password" required autoComplete="new-password" className="mt-1 w-full rounded-xl border border-zinc-700 bg-zinc-900 p-3 text-sm text-white" /></label>
        <label className="block text-xs text-zinc-300">Confirmar senha<input value={confirm} onChange={e => setConfirm(e.target.value)} type="password" required autoComplete="new-password" className="mt-1 w-full rounded-xl border border-zinc-700 bg-zinc-900 p-3 text-sm text-white" /></label>
