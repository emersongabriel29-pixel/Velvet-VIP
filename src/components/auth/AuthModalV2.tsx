  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
    <div className="relative w-full max-w-md rounded-3xl border border-zinc-800 bg-[#121217] p-6 sm:p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
      ...
      {mode === 'login' ? <form onSubmit={submitLogin} className="space-y-4">
        ...
        <label className="block text-xs text-zinc-300">Senha <span className="text-zinc-500">— mínimo de 8 caracteres</span><input value={password} onChange={e => setPassword(e.target.value)} type="password" required minLength={8} autoComplete="new-password" className="mt-1 w-full rounded-xl border border-zinc-700 bg-zinc-900 p-3 text-sm text-white" />{password && <span className={`mt-1.5 block text-[11px] font-semibold ${password.length < 8 ? 'text-rose-400' : password.length < 12 ? 'text-amber-400' : 'text-emerald-400'}`}>{password.length < 8 ? 'Fraca — use pelo menos 8 caracteres' : password.length < 12 ? 'Média' : 'Forte'}</span>}</label>
        <label className="block text-xs text-zinc-300">Confirmar senha<input value={confirm} onChange={e => setConfirm(e.target.value)} type="password" required minLength={8} autoComplete="new-password" className="mt-1 w-full rounded-xl border border-zinc-700 bg-zinc-900 p-3 text-sm text-white" /></label>
