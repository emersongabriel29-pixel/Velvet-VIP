...
import { Radio, Bell, MessageCircle, ShieldCheck, ArrowLeft, Loader2 } from 'lucide-react';
import { isSupabaseConfigured, supabase } from '../../lib/supabase';

type Live = { id: string; title: string; creator_name?: string; status: 'live' | 'scheduled'; scheduled_at?: string; access_tier: 'plus' | 'vip' };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const { data, error } = await supabase.from('live_sessions').select('id,title,status,scheduled_at,access_tier,creator:creators(display_name)').in('status', ['live', 'scheduled']).order('scheduled_at', { ascending: true });
