...
export const LivePage: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const lives = [
    { id: 'live-1', title: 'Noite Velvet — conversa exclusiva', creator: 'Sophia Luxe', viewers: '1,2 mil', status: 'AO VIVO', color: 'from-rose-900 to-purple-950' },
    { id: 'live-2', title: 'Bastidores e perguntas dos fãs', creator: 'Fernanda VIP', viewers: 'Hoje, 21:00', status: 'AGENDADA', color: 'from-zinc-800 to-rose-950' },
    { id: 'live-3', title: 'Especial Clube VIP', creator: 'Valentina Rossi', viewers: 'Amanhã, 20:30', status: 'AGENDADA', color: 'from-amber-950 to-zinc-900' },
  ];
  ...
  return <div className="min-h-screen bg-[#09090b] px-4 pb-28 pt-24 text-white"><div className="mx-auto max-w-5xl">
    ...
    <div className="grid gap-5 md:grid-cols-3">{lives.map(l => <article key={l.id} className={`overflow-hidden rounded-3xl border border-zinc-800 bg-gradient-to-br ${l.color}`}>
      ...
      <div className="bg-black/30 p-4"><h2 className="font-bold">{l.title}</h2><p className="mt-1 text-sm text-zinc-300">{l.creator}</p>{l.status === 'AO VIVO' ? <button className="mt-4 w-full rounded-xl bg-rose-600 py-2.5 text-sm font-bold hover:bg-rose-500">Entrar na live</button> : <button onClick={() => toggle(l.id)} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-zinc-700 py-2.5 text-sm font-bold hover:border-rose-500"><Bell className="h-4 w-4"/> {reminded.includes(l.id) ? 'Lembrete ativado' : 'Lembrar-me'}</button>}</div>
    </article>)}</div>
