...
type Live={id:string;title:string;creator_name?:string;status:'live'|'scheduled';scheduled_at?:string;required_plan:'plus'|'vip';streaming_provider?:string;available_qualities?:string[]};
...
export const LivePage:React.FC<{onBack:()=>void}>=({onBack})=>{
  ...
  const [qualityLabel,setQualityLabel]=useState('Automático');
    ...
    (async()=>{
      try{
        ...
        const {data,error}=await supabase.from('live_sessions')
          .select('id,title,status,scheduled_at,required_plan,streaming_provider,available_qualities,creator:creators(display_name)')
  ...
  const enterLive=async(live:Live)=>{
    ...
    try{
      ...
      setPlayer({live,sources,selected:sources[0]});setHlsQualities([]);setQualityHeight(null);setQualityLabel('Automático');
    }catch(error:any){
  ...
  return <div className="min-h-screen bg-[#09090b] px-4 pb-28 pt-24 text-white"><div className="mx-auto max-w-5xl">
    ...
    <div className="mb-8 flex items-center justify-between gap-4"><div><p className="mb-2 text-xs font-bold uppercase tracking-[0.25em] text-rose-400">Comunidade Velvet</p><h1 className="text-3xl font-black">Lives pagas</h1><p className="mt-2 text-zinc-400">Todas as lives exigem Plus ou VIP; lives exclusivas exigem VIP.</p></div><div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-300"><Radio className="mr-1 inline h-3 w-3"/> {lives.filter(x=>x.status==='live').length} ao vivo</div></div>
    ...
    {loading?<div className="flex justify-center p-12"><Loader2 className="h-6 w-6 animate-spin text-rose-400"/></div>:lives.length===0?<div className="rounded-3xl border border-dashed border-zinc-700 p-12 text-center text-zinc-500">Nenhuma live publicada no momento.</div>:<div className="grid gap-5 md:grid-cols-3">{lives.map(live=><article key={live.id} className="overflow-hidden rounded-3xl border border-zinc-800 bg-gradient-to-br from-rose-950 to-zinc-950"><div className="flex h-44 items-end justify-between p-4"><span className={`rounded-lg px-2 py-1 text-[10px] font-black ${live.status==='live'?'bg-rose-600':'bg-zinc-950/70'}`}>{live.status==='live'?'AO VIVO':'AGENDADA'}</span><span className="rounded-lg bg-black/40 px-2 py-1 text-[10px]">{live.required_plan.toUpperCase()}</span></div><div className="bg-black/30 p-4"><h2 className="font-bold">{live.title}</h2><p className="mt-1 text-sm text-zinc-300">{live.creator_name||'Criador verificado'}</p><p className="mt-2 text-xs font-semibold text-amber-300">Acesso pago • {live.required_plan==='vip'?'VIP':'Plus ou VIP'}</p>{live.status==='live'?<button onClick={()=>enterLive(live)} disabled={joining===live.id} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-rose-600 py-2.5 text-sm font-bold hover:bg-rose-500 disabled:opacity-50">{joining===live.id?<Loader2 className="h-4 w-4 animate-spin"/>:<Radio className="h-4 w-4"/>} Entrar na live</button>:<button onClick={()=>toggle(live.id)} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-zinc-700 py-2.5 text-sm font-bold hover:border-rose-500"><Bell className="h-4 w-4"/> {reminded.includes(live.id)?'Lembrete ativado':formatDate(live.scheduled_at)}</button>}</div></article>)}</div>}
  ...
  {player&&<div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/90 p-3 backdrop-blur-sm"><div className="w-full max-w-5xl overflow-hidden rounded-3xl border border-zinc-800 bg-[#111116]"><div className="flex items-center justify-between border-b border-zinc-800 p-4"><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-rose-400">AO VIVO</p><h2 className="font-bold">{player.live.title}</h2></div><div className="flex items-center gap-2">{(hlsQualities.length>0||player.sources.length>1)&&<div className="relative"><button onClick={()=>setQualityOpen(v=>!v)} className="rounded-xl border border-zinc-700 p-2" title="Qualidade"><Settings className="h-4 w-4"/></button>{qualityOpen&&<div className="absolute right-0 top-11 z-20 w-36 rounded-xl border border-zinc-700 bg-black p-1"><button onClick={()=>{setQualityHeight(null);setQualityLabel('Automático');setPlayer({...player,selected:player.sources[0]});setQualityOpen(false)}} className={`block w-full rounded-lg px-3 py-2 text-left text-xs ${qualityLabel==='Automático'?'bg-rose-600':'hover:bg-zinc-800'}`}>Automático</button>{hlsQualities.map(height=><button key={height} onClick={()=>{setQualityHeight(height);setQualityLabel(height+'p');setQualityOpen(false)}} className={`block w-full rounded-lg px-3 py-2 text-left text-xs ${qualityLabel===height+'p'?'bg-rose-600':'hover:bg-zinc-800'}`}>{height}p</button>)}{player.sources.slice(1).map(src=><button key={src.label+src.url} onClick={()=>{setQualityHeight(null);setQualityLabel(src.label);setPlayer({...player,selected:src});setQualityOpen(false)}} className={`block w-full rounded-lg px-3 py-2 text-left text-xs ${qualityLabel===src.label?'bg-rose-600':'hover:bg-zinc-800'}`}>{src.label}</button>)}</div>}</div>}<button onClick={()=>{setPlayer(null);setQualityOpen(false)}} className="rounded-xl border border-zinc-700 p-2"><X className="h-4 w-4"/></button></div></div><AdaptiveVideo key={player.selected.url} sourceUrl={player.selected.url} sourceType={player.selected.type} qualityHeight={qualityHeight} onQualities={setHlsQualities} controls autoPlay playsInline className="aspect-video w-full bg-black object-contain" onPlaybackError={(message)=>setFeedback(message)}/><div className="p-3 text-xs text-zinc-500">Qualidade: {qualityLabel}. As resoluções manuais vêm do manifesto HLS real.</div></div></div>}
  </div>;
};
