...
DECLARE requested_role TEXT := COALESCE(NEW.raw_user_meta_data->>'requested_role', 'user'); safe_role TEXT := CASE WHEN requested_role = 'creator' THEN 'creator' ELSE 'user' END; birth DATE := NULLIF(NEW.raw_user_meta_data->>'birth_date', '')::DATE;
BEGIN
  IF birth IS NULL OR EXTRACT(YEAR FROM age(current_date, birth)) < 18 THEN RAISE EXCEPTION 'User must be at least 18 years old'; END IF;
  INSERT INTO public.profiles (id, username, name, birth_date, role, age_verified, age_verified_at)
  VALUES (NEW.id, COALESCE(NULLIF(NEW.raw_user_meta_data->>'username', ''), split_part(NEW.email, '@', 1)), COALESCE(NULLIF(NEW.raw_user_meta_data->>'name', ''), split_part(NEW.email, '@', 1)), birth, safe_role, TRUE, NOW());
  IF safe_role = 'creator' THEN
    INSERT INTO public.creators (user_id, display_name, handle, avatar_url, is_approved)
    VALUES (NEW.id, COALESCE(NULLIF(NEW.raw_user_meta_data->>'name', ''), split_part(NEW.email, '@', 1)), COALESCE(NULLIF(NEW.raw_user_meta_data->>'username', ''), split_part(NEW.email, '@', 1)), 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=300&fit=crop', FALSE);
  END IF;
  RETURN NEW;
END; $$;
...
CREATE TABLE IF NOT EXISTS public.wallet_ledger (id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE, payment_id UUID REFERENCES public.payments(id) ON DELETE SET NULL, entry_type TEXT NOT NULL CHECK (entry_type IN ('deposit','purchase','refund','creator_credit','withdrawal','adjustment')), amount NUMERIC(12,2) NOT NULL CHECK (amount <> 0), reference_id TEXT, metadata JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
...
CREATE TABLE IF NOT EXISTS public.webhook_events (id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), provider TEXT NOT NULL, external_event_id TEXT NOT NULL, event_type TEXT, payload JSONB NOT NULL, processed_at TIMESTAMPTZ, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), UNIQUE(provider, external_event_id));
ALTER TABLE public.webhook_events ENABLE ROW LEVEL SECURITY;
CREATE TABLE IF NOT EXISTS public.audit_logs (id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), actor_id UUID REFERENCES public.profiles(id) ON DELETE SET NULL, action TEXT NOT NULL, target_type TEXT, target_id UUID, metadata JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
ALTER TABLE public.audit_logs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "admins read audit logs" ON public.audit_logs;
CREATE POLICY "admins read audit logs" ON public.audit_logs FOR SELECT USING (public.is_admin());

INSERT INTO storage.buckets (id, name, public) VALUES ('velvet-media', 'velvet-media', FALSE) ON CONFLICT (id) DO UPDATE SET public = FALSE;
DROP POLICY IF EXISTS "creators upload velvet media" ON storage.objects;
CREATE POLICY "creators upload velvet media" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'velvet-media' AND EXISTS (SELECT 1 FROM public.creators c WHERE c.user_id = auth.uid() AND c.is_approved = true));
DROP POLICY IF EXISTS "creators manage own velvet media" ON storage.objects;
CREATE POLICY "creators manage own velvet media" ON storage.objects FOR UPDATE TO authenticated USING (bucket_id = 'velvet-media' AND owner_id = auth.uid()::text) WITH CHECK (bucket_id = 'velvet-media' AND owner_id = auth.uid()::text);
DROP POLICY IF EXISTS "creators delete own velvet media" ON storage.objects;
CREATE POLICY "creators delete own velvet media" ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'velvet-media' AND owner_id = auth.uid()::text);
...
BEGIN
  ...
  INSERT INTO public.wallet_ledger(user_id, entry_type, amount, reference_id, metadata) VALUES (p_user_id, p_entry_type, p_amount, p_reference_id, p_metadata) RETURNING id INTO entry_id;
...
BEGIN
  ...
  INSERT INTO public.wallet_ledger(user_id, entry_type, amount, reference_id, metadata) VALUES (creator_user, 'creator_credit', net, p_reference_id, p_metadata) RETURNING id INTO creator_entry;
  ...
  INSERT INTO public.creator_balance(creator_id, available_amount, pending_amount, total_withdrawn) VALUES (p_creator_id, net, 0, 0) ON CONFLICT (creator_id) DO UPDATE SET available_amount = public.creator_balance.available_amount + net, updated_at = NOW();
