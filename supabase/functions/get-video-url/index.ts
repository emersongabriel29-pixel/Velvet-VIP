...
const cloudflareToken=async(assetId:string)=>{
  const accountId=Deno.env.get('CLOUDFLARE_ACCOUNT_ID'),apiToken=Deno.env.get('CLOUDFLARE_STREAM_API_TOKEN');
  if(!accountId||!apiToken) return null;
  const res=await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/stream/${encodeURIComponent(assetId)}/token`,{
  ...
  return res.ok&&body?.success&&body?.result?.token?String(body.result.token):null;
};
const signCloudflareManifest=async(ref:string,assetId:string)=>{
  const token=await cloudflareToken(assetId);
  if(!token)return null;
  try{
    const u=new URL(ref);
    if(!allowedStreamingHosts.has(u.hostname.toLowerCase()))return null;
    u.pathname=u.pathname.replace(`/${assetId}/`,`/${token}/`);
    return u.toString();
  }catch{return null;}
};
...
Deno.serve(async(req)=>{
  ...
  const {data:job}=await admin.from('media_processing_jobs').select('renditions,hls_manifest_path,status,provider,provider_asset_id').eq('video_id',videoId).eq('status','ready').order('updated_at',{ascending:false}).limit(1).maybeSingle();
  ...
  if(job?.provider==='cloudflare'&&job?.provider_asset_id&&typeof manifestRef==='string'&&manifestRef.startsWith('https://')){
    manifest=await signCloudflareManifest(manifestRef,job.provider_asset_id);
    if(!manifest)return response(req,{error:'Secure provider playback unavailable'},503);
  }else{
    manifest=manifestRef?await resolveRef(manifestRef):null;
  }
  const autoUrl=manifest||original;
  const sources=manifest
    ? [{label:'Automático',height:0,url:manifest,type:'application/vnd.apple.mpegurl'},...renditions.filter((x,i,a)=>i===a.findIndex(y=>y.label===x.label))]
    : [{label:'Automático',height:0,url:original,type:'video/mp4'},...renditions.filter((x,i,a)=>i===a.findIndex(y=>y.label===x.label))];

  return response(req,{url:autoUrl,sources,adaptive_manifest:manifest,expires_in:manifest?300:120,anonymous:anonymousAllowed,adaptive:Boolean(manifest),provider:job?.provider||null});
});
