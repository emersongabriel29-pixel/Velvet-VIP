...
Deno.serve(async (req) => {
  ...
  if (!supabaseUrl || !anonKey || !serviceKey || !mpToken) return json({ error: 'Checkout não configurado no servidor.' }, 503);
  ...
  if (kind === 'platform_plan' && body.planId) {
    ...
    if (!plan?.is_active || Number(plan.monthly_price) <= 0) return json({ error: 'Plano inválido.' }, 400);
    ...
    if (!plan?.is_active) return json({ error: 'Plano do criador inválido.' }, 400);
    ...
    if (!Number.isFinite(amount) || amount < 1 || amount > 9999) return json({ error: 'Valor de gorjeta inválido.' }, 400);
    ...
    return json({ error: 'Dados de checkout incompletos.' }, 400);
  }
  ...
  if (sessionError || !session) return json({ error: 'Não foi possível criar a sessão.' }, 500);
  ...
  if (!mpResponse.ok) {
    ...
    return json({ error: 'Gateway recusou o checkout.' }, 502);
  }
