...
test('media bucket stays private with constrained MIME and size',()=>{assert.match(sql,/public,false,536870912/);assert.match(sql,/allowed_mime_types/);});
...
test('playback uses short lived signed URLs only',()=>{assert.match(edge,/createSignedUrl\(path, 120\)/);assert.match(edge,/Unmanaged media URL rejected/);assert.match(edge,/Cache-Control': 'no-store'/);});
