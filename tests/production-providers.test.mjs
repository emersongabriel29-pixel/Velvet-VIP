...
const sql=fs.readFileSync('supabase/migrations/028_production_stage3_provider_adapters.sql','utf8');
...
test('provider credentials stay server-side',()=>{assert.match(env,/STREAMING_API_KEY/);assert.doesNotMatch(env,/VITE_STREAMING_API_KEY/);assert.match(docs,/No streaming secret is exposed to Vite/);});
